...
$nip = $_POST['nip'];
$stmt = $con -> getConnection() -> prepare("SELECT * FROM customers WHERE nip = ?");
...
if (mysqli_num_rows($res) != 1){
    if(!isset($_POST['is_in'])){
        ...
        $stmt = $con -> getConnection() -> prepare("INSERT INTO `customers`(`nip`, `company_name`, `zip`, `street`, `apartment`, `town`) VALUES(?,?,?,?,?,?)");
...
#adding invoice
$stmt = $con -> getConnection() -> prepare("SELECT id FROM customers WHERE nip = ?");
$stmt -> bind_param("i",$nip);
$stmt -> execute();
$id = $stmt -> get_result();
...
$stmt = $con -> getConnection() -> prepare("INSERT INTO `invoices`(`customer_id`, `sale_date`, `payment_date`, `date_of_issue`, `payment_method`, `payed`, `comments`) VALUES (?,?,?,?,?,?,?)");
$stmt -> bind_param("sssssss", $id_value, $sale_date, $payment_date, $date_of_issue, $payment_method, $payed, $comments);
$stmt -> execute();
$stmt -> close();
#get invoice number
$stmt = $con -> getConnection() -> prepare("SELECT MAX(nr) FROM invoices"); #zdaję sobie sprawę z niepoprawności tego rozwiązanie jednak na tem moment tylko takie przychodzi mi do głowy
$stmt -> execute();
...
if (isset($_POST['name_of_item'])){
    ...
    for ($i = 0; $i < count($name_of_item); $i++){
        $stmt = $con -> getConnection() -> prepare("INSERT INTO `rows`(`invoice_nr`, `name`, `quantity`, `unit`, `price_brutto`, `vat`) VALUES (?,?,?,?,?,?)");
